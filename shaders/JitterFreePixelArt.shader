...
uniform float smoothing_factor : hint_range(0.1, 1.0) = .1;

void fragment() {
	// compute the new uv
	vec2 uv = SCREEN_UV;
	vec2 uv_width = fwidth(SCREEN_UV);
	vec2 sprite_screen_resolution = smoothing_factor / uv_width;
		
	vec2 uv_pixel_src = floor(uv / SCREEN_PIXEL_SIZE + 0.499);
	
	vec2 edge = uv_pixel_src;
	edge = edge * SCREEN_PIXEL_SIZE * sprite_screen_resolution;

	vec2 uv_pixel = uv * sprite_screen_resolution;
	vec2 uv_factor = clamp(uv_pixel - edge + 0.5, 0.0, 1.0);

	uv = (mix(uv_pixel_src - 1.0, uv_pixel_src, uv_factor) + 0.5) * SCREEN_PIXEL_SIZE;

	// now we can use the uv as always...
	COLOR = texture(SCREEN_TEXTURE, uv).rgba;
	COLOR.r += .2;
}